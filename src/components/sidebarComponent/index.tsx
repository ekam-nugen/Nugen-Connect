...
export default function SidebarComponent({ }: sidebarItemType) {
  ...
  return (
    <div
      className={`overflow-y-auto sidebar border-4 border-red-700${isCollapsed ? "w-16" : "w-48"
        } mt-2  top-0 border-r transition-all 
      duration-100 ease-in-out`}
    >
      <div
        className={`flex px-2 bg-white ${isCollapsed
            ? "justify-center items-center pb-3 text-center"
            : "justify-end"
          }`}
      >
        <button
          onClick={toggleSidebar}
          className="flex text-gray-500 hover:text-gray-700 h-5 w-5 items-center bg-gray-200 rounded-md"
        >
      ...
      <div className="mb-2 px-2 h-full">
        {OverviewItems.map((item: any, index: number) => (
          <div
            key={index}
            className={`flex items-center mb-2 h-9 gap-2 rounded-md px-1 group hover:bg-gray-100 shadow-x-lg hover:cursor-pointer ${isCollapsed ? "justify-center" : ""
              }`}
          >
            ...
            {isCollapsed && (
              <span className="absolute left-12 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gray-100 text-gray-700 text-nowrap text-sm rounded-md px-2 h-8 py-1 pointer-events-none">
                {item?.label}
              </span>
            )}
          </div>
      ...
      <div className="mb-2 px-2 h-full">
        {userItems?.map((item: any, index: number) => (
          <div
            key={index}
            className={`flex items-center mb-2 h-9 gap-2 rounded-md px-1 hover:bg-gray-100 group hover:cursor-pointer ${isCollapsed ? "justify-center" : ""
              }`}
          >
            <span className="text-xl h-7.5 w-7.5 text-zinc-600 flex items-center justify-center rounded-md px-1 ">
              <item.icon />
      ...
      <div className="mb-4 px-2 h-full items-center">
        {sidebarItems?.map((item: any, index: number) => (
          <div
            key={index}
            className={`flex items-center mb-0.5 h-10 gap-2 rounded-md px-1 hover:bg-gray-100 transition-all group relative hover:cursor-pointer ${isCollapsed
        ...
        <div
          className={`flex items-center mb-auto px-2 h-10 mt-2 hover:text-blue-500 gap-2 rounded-xl ${isCollapsed ? "justify-center" : ""
            }`}
        >
      ...
      <div
        className={`fixed bottom-0 left-0 w-48 border-1 border border-zinc-200 bg-white ${isCollapsed ? "w-16 hidden" : "w-48"
          }`}
      >
        <hr />
