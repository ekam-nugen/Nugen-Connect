          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {quickTask?.map((item, index) => (
              <div key={item.color + index}>
                <Card className="rounded-xl px-3 py-4 flex flex-col items-center justify-center text-center shadow-sm shadow-gray-100 hover:bg-gray-200 cursor-pointer">
                  <CardTitle className={`${item?.color} p-2`}>
                    {item?.icons}
                  </CardTitle>
                  <CardDescription className="text-sm text-gray-600 font-medium text-nowrap">
