...
export function DatePicker({
  ...
  const selectBoxOptions: SelectedOptionType[] = checkInVariant || checkoutVariant ? [
  ...
  const handleDateChange = (selectedDate: DateRange | Date | undefined) => {
    if (selectedDate) {
      if (mode === "range" && "from" in selectedDate && "to" in selectedDate) {
        ...
        handleDateRange && handleDateRange(selectedDate);
  ...
  const reset = () => {
    if (mode === "range") {
      ...
      handleDateRange && handleDateRange(clearedRange);
    } else if (mode === "single") {
      setSingleDate(new Date());
    }
  ...
  const handlePresetSelect = (value: string) => {
    ...
    switch (days) {
      ...
      case 2:
        ...
        break;
      case 3:
        newFromDate = (checkInVariant || checkoutVariant) ? now : subDays(now, 30);
    ...
    if (mode === "range") {
      ...
      handleDateRange && handleDateRange({ from: newFromDate, to: newToDate });
    } else if (mode === "single") {
  ...
  const disabledDates = (date: Date): boolean => {
    ...
    if (mode === "single") {
      if (checkoutVariant && checkInPropValue) {
        return date < today || (checkInPropValue ? normalizeDate(date) === normalizeDate(checkInPropValue) : false)
      }
      else if (checkInVariant) {
        return date < today;
      }
      else {
        return date < today || (singleDate ? isBefore(date, singleDate) : false);
      }
    }
    return date > today
  };
  ...
  return (
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
      ...
      <PopoverContent className="flex w-auto flex-col space-y-2 p-2">
        {!checkoutVariant && <div className={cn("flex justify-between gap-2", mode === "range" && 'gap-2')}>
          <SelectBox
          ...
          />
          {mode === "range" && <Button variant="default" onClick={reset}>
            Reset
          </Button>}
        </div>}
        <div className="rounded-md border">
          {mode === "range" ? (
            ...
            <Calendar
              ...
              numberOfMonths={1}
              disabled={disabledDates}
              fromDate={checkInPropValue ?? (checkInVariant ? new Date() : singleDate)}
            />
          )}
        </div>
      </PopoverContent>
